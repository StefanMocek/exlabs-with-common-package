...
import {
  AuthUserService,
  authUserService,
} from "./auth-user/auth.user.service";
...
export class AuthService {
  ...
  async register(createAuthUserDto: AuthUserDto) {
    const existingAuthUser = await this.authUserService.findOneByEmail(
  ...
  async signin(signInDto: AuthUserDto) {
    ...
    const pwdCompered = await this.authenticationService.passwordCompare(
      authUser.password,
      signInDto.password
    );
